...
class eccDefinition:
    ...
    def get_t_average_for_mean_motion(self):
        ...
        --------
        t_for_mean_motion:
            Times associated with mean motion
        sorted_idx_for_mean_motion:
            Indices used to sort the times associated with mean motion
        """
        ...
        t_for_mean_motion = np.append(
            self.t_average_apocenters,
            self.t_average_pericenters)
        sorted_idx_for_mean_motion = np.argsort(t_for_mean_motion)
        t_for_mean_motion = t_for_mean_motion[sorted_idx_for_mean_motion]
        return t_for_mean_motion, sorted_idx_for_mean_motion
    ...
    def compute_mean_motion_at_extrema(self, t):
        ...
        omega22_mean_motion = np.append(self.omega22_average_apocenters,
                                        self.omega22_average_pericenters)

        # get the times associated to the mean motion
        if not hasattr(self, "t_for_mean_motion"):
            self.t_for_mean_motion, self.sorted_idx_for_mean_motion = \
                self.get_t_average_for_mean_motion()
        # We now sort omega22_average using sorted_idx_for_mean_motion, the
        # same array of indices that was used to obtain the t_for_mean_motion
        # in the function eccDefinition.get_t_average_for_mean_motion.
        omega22_mean_motion = omega22_mean_motion[self.sorted_idx_for_mean_motion]
        # check that omega22_average in strictly monotonic
        self.check_monotonicity_of_omega22_average(
            omega22_mean_motion,
            "omega22 averaged [apocenter to apocenter] and [pericenter to pericenter]")
        return self.interp(
            t, self.t_for_mean_motion, omega22_mean_motion)
    ...
    def get_omega22_average(self, method=None):
        ...
        if method != "mean_motion":
            ...
            self.t_for_mean_motion, self.sorted_idx_for_mean_motion = \
                self.get_t_average_for_mean_motion()
            # for mean motion, the associated times are obtained using the temporal
            # midpoints of the extrema, therefore we need to make sure that we use
            # only those times that fall within tmin and tmax.
            self.tmin_for_fref = max(self.tmin, min(self.t_for_mean_motion))
            self.tmax_for_fref = min(self.tmax, max(self.t_for_mean_motion))
    ...
    def check_omega22_average(self):
        """Check that orbit average of omega22_average is equal to omega22_average.

        When constructing the omega22_average, we associate the orbit averaged omega22
        with a time at the midpoint between the two extrema which defines the current
        orbit over which the averaging takes place. This association of the temporal
        midpoint with the average omega22 is somewhat adhoc and therefore we need to
        check how good is this choice of the associated time.

        To check this, after we have obtained the orbit averaged omeg22, i.e.,
        omega22_average we take an orbit average of omega22_average between two
        extrema and call this second_omega22_average.  We then check how close
        the second_omega22_average is to the omega22_average at the midpoint
        ...
        # check if the t_for_omega22_average starts at midpoint between pericenrers or apoceneters
        if self.t_for_omega22_average[0] == self.t_average_pericenters[0]:
            # since t_for_omega22_average starts at midpoint between
            # pericenrers i.e. close to an apoceneter, we will do averaging of
            # omega22_average between the apoceneters
            t_extrema = self.t_apocenters
            # we will compare the average of omeg22_average with the omega22_average
            # at the midpoints between the apoceneters which we already know to be
            # given by the omega22_average_apoceneters
            omega22_average = self.omega22_average_apocenters
            midpoint_between = "apocenters"
        else:
            midpoint_between = "pericenters"
            t_extrema = self.t_pericenters
            omega22_average = self.omega22_average_pericenters
        second_omega22_average = []
        t_midpoints = []
        for idx in range(len(t_extrema)-1):
            # we find the start and end representing the indices the start and end
            # of an orbit between two consecutive extrema.
            start = np.argmin(np.abs(self.t_for_omega22_average - t_extrema[idx]))
            end = np.argmin(np.abs(self.t_for_omega22_average - t_extrema[idx+1]))
            # get the orbital average of omega22_average
            avg = (np.trapz(self.omega22_average[start:end],
                            self.t_for_omega22_average[start:end])/
                   (self.t_for_omega22_average[end] -
                    self.t_for_omega22_average[start]))
            second_omega22_average.append(avg)
            # get the time at the midpoint between the two extrema
            t_midpoints.append(0.5 * (self.t_for_omega22_average[end] +
                                      self.t_for_omega22_average[start]))
        t_midpoints = np.array(t_midpoints)
        second_omega22_average = np.array(second_omega22_average)
        style = "Notebook"
        use_fancy_plotsettings(style=style)
        nrows = 2
        fig, axes = plt.subplots(nrows=nrows, sharex=True,
                                 figsize=(figWidthsTwoColDict[style],
                                          nrows * figHeightsDict[style]))
        axes[0].plot(self.t, self.omega22, c="k", alpha=0.5, lw=0.5)
        axes[0].plot(self.t_average_apocenters, self.omega22_average_apocenters,
                     label=labelsDict["omega22_average_apocenters"],
                     marker=".",
                     c=colorsDict["apocenter"],
                     ls="")
        axes[0].plot(self.t_average_pericenters, self.omega22_average_pericenters,
                     label=labelsDict["omega22_average_pericenters"],
                     marker=".",
                     c=colorsDict["pericenter"],
                     ls="")
        label = r"\mathrm{p}" if midpoint_between == "pericenters" else r"\mathrm{a}"
        axes[0].plot(t_midpoints, second_omega22_average,
                     label=fr"$\langle\langle \omega_{{22}}\rangle\rangle^{label}$",
                     marker="+", ls="")
        axes[0].legend()
        axes[1].semilogy(t_midpoints, abs(omega22_average - second_omega22_average),
                         marker=".", ls="")
        axes[1].set_xlabel(labelsDict["t"])
        axes[1].set_ylabel(r"$\Delta\langle\omega_{22}\rangle$")
        plt.subplots_adjust(hspace=0.02)
        return omega22_average, second_omega22_average
