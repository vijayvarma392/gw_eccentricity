...
def get_load_waveform_defaults(origin="LAL"):
    ...
    """
    origins = get_available_waveform_origins()
    if origin not in origins:
        raise Exception(f"Unknown origin {origin}. "
                        f"Must be one of {list(origins)}.")
...
def load_waveform(origin="LAL", **kwargs):
    ...
    origin: str
        ...
        `load_data.get_available_waveform_origins`. See the waveform
        loading function called by a given `origin` for detail
        documentation.
    ...
    kwargs:
        ...
        The possible set of kwargs and the default values for an `origin`
        can be obtained using `load_data.get_load_waveform_defaults`.
    Returns:
    --------
    dataDict:
...
def load_LAL_waveform(**kwargs):
    ...
    NOTE: The default values are set using values from
    `load_data.get_load_waveform_defaults` with origin = "LAL".  Use
    it to get the dictionary of possible kwargs and their default
    values.
    approximant: str
...
def load_lvcnr_waveform(**kwargs):
    ...
    `load_data.get_load_waveform_defaults` with origin = `LVCNR`.
    Use this function to get the dictionary of possible
...
def load_sxs_catalogformat(**kwargs):
    ...
    kwargs: Dictionary with the followings keys.
     NOTE: The default values are set using
    `load_data.get_load_waveform_defaults` with origin = `SXSCatalog`.
    Use this function to get the dictionary of possible
    kwargs and default values.
    filepath: str
...
def load_EOB_waveform(**kwargs):
    ...
    NOTE: The default values are set using
    `load_data.get_load_waveform_defaults` with origin = `EOB`.
    Use this function to get the dictionary of possible
    kwargs and default values.
...
def load_lvcnr_hack(**kwargs):
    ...
    ----------
    kwargs: Could be the followings.
    NOTE: Defaults are set using `load_data.get_load_waveform_defaults` with
    origin = `LVCNR_hack`. Use this function to get dictionary of default
    values.
    filepath: str
        Path to lvcnr file.
...
def load_EMRI_waveform(**kwargs):
    ...
    NOTE: The default keys are set using `load_data.get_load_waveform_defaults`
    with orgin="EMRI". This function can be used to get the dictionary
    of possible kwargs and their default values.
    filepath: str
