...
def check_kwargs_and_set_defaults_for_nr(kwargs, allowed_kwargs_list,
                                         location):
    """Check keys in kwargs and set dafults.

    Parameters:
    -----------
    kwargs: dict
        Dictionary of kwargs to check and set default values.
        The keys in `kwargs` must be one of the `allowed_kwargs_list` (see below).
        The keys that are in `allowed_kwargs_list` but not in `kwargs` are
        inserted in `kwargs` with defaults values set in this function.
    ...
    for kw in kwargs:
        if kw not in allowed_kwargs_list:
            raise Exception(f"Unknown key {kw}. "
                            ...
                            "To change the allowed list of keys, "
                            f"modify the allowed kwargs in {location}.")
