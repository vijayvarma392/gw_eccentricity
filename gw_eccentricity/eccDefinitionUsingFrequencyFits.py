...
class eccDefinitionUsingFrequencyFits(eccDefinition):
    ...
    def find_extrema(self, extrema_type="maxima"):
        ...
        if extrema_type in ['maxima', "pericenter", 'peaks']:
            sign = +1
        elif extrema_type in ['minima', "apocenter", 'troughs']:
            sign = -1
        else:
        ...
        # omega22_extrema_refined, phase22_extrema_refined] and/or
        # self.apastron_info = [t_extrema_refined, omega22_extrema_refined,
        ...
        if verbose:
            diag_file = (
                f"Diagnostics-FrequencyFits-{ {-1:'minima', 1:'maxima'}[sign]}"
                ".pdf")
        else:
            diag_file = ""
        ...
        if pp:
            ...
            axs[2].plot(self.t_analyse,
                        self.omega22_analyse-f_fit(self.t_analyse, *p_global),
                ...
                if terminate:
                    ...
                    for k in range(N+1, len(idx_extrema)):
                        if idx_extrema[k] > idx_ref:
                            extrema.append(idx_extrema[k])
                            t_extrema_refined.append(extrema_refined[0][k])
                            omega22_extrema_refined.append(
                                extrema_refined[1][k])
...
def FindExtremaNearIdxRef(t, phase22, omega22,
    ...
    where
      ...
      - extrema_refined=(t_extrema_refined, omega22_extrema_refined,
    ...
    # this variable counts the number of iterations in which Nright was one too
    # few.  This is used to detect limiting cycles, where the interval
    ...
    if pp:
        ...
        axs[0].set_title(
            'trend-subtracted:  sign*(omega-f_fit)',
            fontsize='small')
    ...
    while True:
        ...
        if prominence is None:
            ...
            T_orbit = (t[idx_hi-1] - t[idx_lo])/(
                phase22[idx_hi-1] - phase22[idx_lo]) * 4*np.pi
        ...
        # omega_residual is shorter array
        omega_residual_extrema = omega_residual[idx_extrema-idx_lo]
        ...
        if refine_extrema:
            ...
            # accuracy
            if verbose:
                print(f"       Refineme {N_extrema} extremas, local fit with "
                      "Npoints=", end='')
            for k in range(N_extrema):
                ...
                if N_refine >= 7:  # enough data for fit
                    ...
                    omg_resi_parafit = omega22[idx_refine] - f_fit(t_parafit,
                                                                   *p)

                    parabola = np.polynomial.polynomial.Polynomial.fit(
                        t_parafit, omg_resi_parafit, 2)
                    t_max = parabola.deriv().roots()[0]

                    # update extrema information
                    t_extrema[k] = t_max

                    # interpolate omega from fits *assumption* the
                    # fitting-interval is short enough that this is accurate
                    omega22_extrema[k] = parabola(t_max) + f_fit(t_max, *p)

                    # 3rd order fit to phase to interpolate *assumption* the
                    # fitting-interval is short enough that this is accurate
                    phase_fit = np.polynomial.polynomial.Polynomial.fit(
                        t_parafit, phase22[idx_refine], 3)
        ...
        if pp:
            ...
            line, = axs[0].plot(t[idx_lo:idx_hi],
                                it*plot_offset
                                + sign*omega_residual, label=f"it={it}")
            if N_extrema > 0:
                axs[0].plot(t_extrema,
                            it*plot_offset+sign*omega_residual_extrema,
                            'o',
                            color=line.get_color())
            line, = axs[1].plot(t[idx_lo:idx_hi],
                                omega22[idx_lo:idx_hi]+plot_offset*it)
            axs[1].plot(t_extrema, omega22_extrema+plot_offset*it, 'o',
        ...
        if N_extrema == 0 or it > 20:
            if verbose:
                if N_extrema == 0:
                    print("could not identify a single extremum. "
                          ...
                          "for low eccentricity late in the inspiral where the"
                          " range of omega\n"
                          "is so large that the prominence = "
                          f"{0.03*omega_residual_amp} cannot be\n"
                          "reached by the small eccentricity oscillations")
                else:
                    print(f"interval finding failed after it={it} iterations. "
                          "Exit")
            if pp:
                # plt.legend()
                fig.tight_layout()
            ...
            # so return with empty idx_extrema and let upstream code handle
            # this
            return idx_extrema, p, K, idx_ref, [t_extrema, omega22_extrema,
                                                phase22_extrema]
        ...
        if Nleft != Nbefore or Nright != Nafter:
                ...
                if idx_lo == 0:
                    ...
                    if increase_idx_ref_if_needed:
                        if Nright >= 2:
                            ...
                            # widen the interval.  The next iteration will come
                            # back here and update idx_ref
                    ...
                    # idx_lo toward seeing one earlier extremum.  Rationale for
                    # 0.6: The next extremum should be 1 radial period earlier.
                    # We rather prefer to err on the low side, than
                    # overshooting and adding two extrema at once.
            ...
            if Nright > Nafter:
                ...
                if idx_hi < len(phase22):
                    # target phase on right 0.6 radial periods beyond current
                    # end of interval rationale for 0.6: The next extremum
                    # should be 1 radial period away, we are worried that near
                    # the end of the run, this prediction may not be accurate.
        ...
        #      * positions idx_extrema If refine_extrema==True: the arrays are
        #      * refined via fits.
        #
        ...
        if it >= 16 \
           ...
           and interval_changed_on_it == it-1:
            ...
            # N_extrema points toward a limiting cycle.  Since we just happened
            # to have hit the right number of extrema, let's take them and
            # exit.
            if verbose:
        ...
        if Count_Nright_short >= 5 or N_extrema < 5 or it > 20:
            ...
            # we require **5** extrema, in order to have safety for the **3**
            # parameter fit below
            if verbose:
                print(f"exiting because Count_right_short={Count_Nright_short}"
                      f" is large, or N_extrema={N_extrema} is insufficient")
            if pp:
                # plt.legend()
                fig.tight_layout()
                fig.savefig(pp, format='pdf')
        ...
        if max_delta_omega < TOL:
            # (this cannot trigger on first iteration, due to initialization of
            # old_extrema)
            if verbose:
                print(f"max_delta_omega={max_delta_omega:5.4g}<TOL={TOL}.  "
                      "Done")
            if pp:
                # plt.legend()
                fig.tight_layout()
        ...
        if verbose and False:
            print("    PRODUCTION FIT: residual="
                  f"{np.linalg.norm(f_fit(t_extrema,*p)-omega22_extrema)},"
                  f" f={f_fit.format(*p)}")
        ...
        if verbose:
            print(f"       max_delta_omega={max_delta_omega:5.4g} => "
                  "fit updated to"
